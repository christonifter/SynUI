...
%             figure(5+i);clf; pspectrum(pst(i).bincount(:, data.chanlist), binfs, 'FrequencyLimits', [0 100]);
        ...
        %LFP
            ...
            for j = 1:numel(subspikes)
                if app.ClustsCheck.Value
                    pC = data.cluster(subclusters(j)).peakChannel2(1);
                else
                    pC = subclusters(j);
                end
                plot(lax(i), subspikes(j)-ref, -pC , 'ro')
            end
            hold(lax(i), 'off');
        end %for PSTH1 and 2
...
%                 t = (1:size(data.raw, 1))./data.fs;
    ...
    out.statstable = table(data.chanlist, averate(1,:)', averate(2,:)', CI5(1,:)', CI5(2,:)',...
        CI95(1,:)', CI95(2,:)', PSTH2pmax', PSTH2plat', ...
        'VariableNames', {'Channel', 'PSTH1AveRate', 'PSTH2AveRate', 'PSTH1CI5', 'PSTH2CI5', 'PSTH1CI95', 'PSTH2CI95', 'PSTH2MaxRate', 'PSTH2MaxLatency'});
    if ~isempty(VS2)
    ...
    if app.ClustsCheck.Value
        ...
        for i = 1:numel(data.chanlist)
            spikesi = cluster(data.channelsortorder(data.chanlist(i))).spikes;
        ...
        out.statstable = addvars(out.statstable, peakChannel2, ADdiffl, ADndiffl, ADzl, ADdiff, ADndiff, ADz, ...
            'NewVariableNames', {'Nearest_Channel', 'PSTH_AD_Difference', 'PSTH_AD_normalized_diff', 'PSTH_AD_zscore', ...
            'Total_AD_Difference', 'Total_AD_normalized_diff', 'Total_AD_zscore'});
    end