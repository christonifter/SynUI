...
%tuning curve
    ...
    if numel(unique(data.frqs))>1 || numel(unique(data.lvls)) > 1 %not LDS
        spontwin = [-0.2 0];
        spontrates = findspontratev2(data.spets, data.stimons, data.channels, data.chanlist, spontwin);
        totaltime = (spontwin(2)-spontwin(1))*numel(data.stimons);
...
%running spike rate average
    ...
    recpos = round([app.PSTH1StartEdit.Value + ref, -100, app.PSTH1EndEdit.Value-app.PSTH1StartEdit.Value, 100]); 
    rectangle(ax, 'Position', recpos, 'FaceColor', [.8 .8 1], 'EdgeColor', [.8 .8 1]);
    ...
    recpos = round([app.PSTH2StartEdit.Value + ref, -100, app.PSTH2EndEdit.Value-app.PSTH2StartEdit.Value, 100]);
    rectangle(ax, 'Position', recpos, 'FaceColor', [1 .8 .8], 'EdgeColor', [1 .8 .8]);
    if isfield(app.data, 'evons')
        if numel(app.data.evons>0)
            if numel(app.data.evoffs)<numel(app.data.evons)
                evp = [app.data.evons [app.data.evoffs; max(app.data.evons)]];
            else
                evp = [app.data.evons app.data.evoffs];
            end
            if size(evp,1) > 400
                counterinterval = ceil(size(evp,1)/100);
            else
                counterinterval = 1;
            end

            for ev = 1:counterinterval:size(evp, 1)
                plot(ax, evp(ev, 1), -.1, 'r.', 'LineWidth', 4);
    %             rectangle(ax, 'Position', [evp(ev, 1) -100  evp(ev, 2)-evp(ev, 1) 100], 'FaceColor', .8*[1 1 1], 'EdgeColor', .8*[1 1 1])
            end
        end
    end
    ...
    if app.ClustsCheck.Value
        set(ax, 'YTick', -numel(app.data.cluster):-1);
