function synuiplotting(app)
            ...
            if app.ClustsCheck.Value
                ...
                %cluster of each spike is stored in data.clusters and data.channels
                ...
                %data.chanlist contains the channel indices selected for plotting
                %in ascending order
                %data.channelsortorder(data.chanlist) contains the actual
                %channel numbers selected for plotting
                ...
                %However, the LFP raster requires the original cluster code.
            else
...
%tuning curve
            ...
            elseif numel(unique(data.frqs))>1 && numel(unique(data.lvls)) == 1 %ISO-I
                ...
                ftcstats = ftcthreshold(ftc, sort(unique(data.frqs)), poissinv(0.95, spontrates), 10);
                if app.ClustsCheck.Value
            ...
            elseif numel(unique(data.frqs))==1 && numel(unique(data.lvls)) > 1 %ISO-F
                ...
                ftcstats = ftcthreshold(ftc, sort(unique(data.lvls)), poissinv(0.95, spontrates), 10);
                if app.ClustsCheck.Value
