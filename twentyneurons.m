...
% clus2 = 20;
%     for sampi = 1:51
%         by(sampi,:) = hist([cluster(clus1).snips(:,sampi,peakChannel2(clus1)); cluster(clus2).snips(:,sampi,peakChannel2(clus1))], -2:.1:2);
%     end
% 
%     ebl = [cluster(clus1).msnip(peakChannel(clus1),:) - channelnoise(peakChannel(clus1)), ...
%         cluster(clus1).msnip(peakChannel(clus1),:) + channelnoise(peakChannel(clus1))];
%     ebu = [cluster(clus2).msnip(peakChannel(clus1),:) - channelnoise(peakChannel(clus1)), ...
%         cluster(clus2).msnip(peakChannel(clus1),:) + channelnoise(peakChannel(clus1))];
%     plot(0,0);
%     hold on;
%     imagesc(-by'); colormap(gray)
%     plot(ebl.*10+20, 'b:', 'LineWidth', 2);
%     plot(ebu.*10+20, 'r:', 'LineWidth', 2);
%     hold off;
%     [u,d] = findcross2(ebl(:,1) - ebu(:,2));
%     [u2,d2] = findcross2(ebl(:,2) - ebu(:,1));
