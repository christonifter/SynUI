...
function livestreamapp3(app, mode) 
...
if app.syn.getMode() == 0
    ...
    if sum(ismember(app.pars, 'Frequency')) == 0
        if sum(ismember(app.pars, 'HPCF')) == 0
            if sum(ismember(app.pars, 'Level')) == 0
                load('FRA.mat')
            else
                load('Iso-I.mat')
            end
...
%read triggered data
    ...
    if isfield(new.data.epocs, stimvar2)
