...
%             figure(5+i);clf; pspectrum(pst(i).bincount(:, data.chanlist), binfs, 'FrequencyLimits', [0 100]);
        ...
        %LFP
            ...
            switch lower(app.(reffield).Value)
                case '0'
                    ref = 0;
                case 'onset'
                    ref = data.stimons(1);
...
%Average cycles
        ...
        LDSaverate = LDSspikecount./LDSduration;
        try
            spontwin = [app.BaselineStartEdit.Value app.BaselineEndEdit.Value];
            spontrates = findspontrate(data.spets, data.evons, data.channels, data.chanlist, spontwin); %total spikes
            totaltime = (spontwin(2)-spontwin(1))*numel(data.evons);
            spontrates = spontrates./totaltime; %spikes 
            spontsig = poissinv(0.95, spontrates*2.5)/2.5;
        catch ME
        end
    ...
    %find train
        trainstarts = [1; find(diff(data.evons)>1.5*period)+1];
        trainends = [find(diff(data.evons)>1.5*period); numel(data.evons)];
        traindur = data.evons(trainends(1))-data.evons(trainstarts(1));
    ...
    %align spike times to evoke pulse times
    %select only spikes from selected time windows
            [~, ~, PSTHspets, ~] = freqlevelspet(data.spets(selection), data.evons, data.frqs, data.lvls); 
            [~, ~, tPSTHspets, ~] = freqlevelspet(data.spets(selection), data.evons(trainstarts), data.frqs(trainstarts), data.lvls(trainstarts)); 
    %select only spikes from selected channels
            ...
            trainpst(i).PSTHchans = selchannels(PSTHsel); 
            trainpst(i).PSTHspets = tPSTHspets(PSTHsel);
            for chan = 1:numel(data.chanlist)
                [y, x] = hist(trainpst(i).PSTHspets(pst(i).PSTHchans==data.chanlist(chan)), 0:psthwindow:traindur);
                trainpst(i).bincount(:, chan) = y(1:(end-1));
                trainpst(i).bintime(:,chan) = x(1:(end-1));
            end

%             [tempst(i), ~, ~, ~] = synpstbin(app, data, i, spetfreq, str2double(app.FrequencyDrop.Value), spetlevel, targetlevel);
            psthwindow = app.PSTHBinEdit.Value./1000; 
            binfs = 1/psthwindow;
             [p(i,:,:), f] = pspectrum(trainpst(i).bincount, binfs, 'FrequencyLimits', [0 200]);
            ...
            app.data.sounddriven = sounddriven;
            ...
            for chan = 1:numel(data.chanlist)
                ...
                jitter(chan, i) = std(pst(i).PSTHspets(pst(i).PSTHchans==data.chanlist(chan)));
...
%count spikes in UI defined analysis window
                ...
                pst(i).baselinecount(chan, 1) = sum(pst(i).PSTHspets(pst(i).PSTHchans==data.chanlist(chan)) > app.BaselineStartEdit.Value*1E-3 & ...
                    pst(i).PSTHspets(pst(i).PSTHchans==data.chanlist(chan)) < app.BaselineEndEdit.Value*1E-3);
%estimate baseline spiking from first 7 ms
                cbaseline = sum(pst(i).PSTHspets(pst(i).PSTHchans==data.chanlist(chan)) < 7E-3)*app.PSTHBinEdit.Value/7;
                pp = app.CLEdit.Value/100;
                thresh(chan, i) = (1-pp)*(max(bincount(:,chan))) + pp*cbaseline;
                [u, d] = findcross2(bincount(:,chan) - thresh(chan, i));
                firstafterbaseline = find(u > 7/app.PSTHBinEdit.Value, 1);
                if numel(firstafterbaseline ) > 0 & numel(d) > 0
                    onset(chan, i) = u(firstafterbaseline).*app.PSTHBinEdit.Value;
                    offset(chan, i) = d(end).*app.PSTHBinEdit.Value;
                else
                    onset(chan, i) = NaN;
                    offset(chan, i) = NaN;
                end
            end
            analduration = (app.PSTHOffsetEdit.Value-app.PSTHOnsetEdit.Value)*1E-3;
            baselineduration = (app.BaselineEndEdit.Value-app.BaselineStartEdit.Value)*1E-3;
            pst(i).analrate = pst(i).analcount./analduration./ncycles;
            pst(i).baselinerate = pst(i).baselinecount./baselineduration./ncycles;
            VS2(:,i) = VS;
...
%find peak rate/latency   
    psthwindow = app.BinEdit.Value./1000;
...
%LDS-TN
            ...
            hasntmultiplespikes = find(sum(trainpst(1).bincount>0) < 1.5 | sum(trainpst(2).bincount>0) < 1.5);
            modgaindb = 10*log10(AC(2,:)'./AC(1,:)');
            modgaindb(hasntmultiplespikes) = NaN;
            out.statstable = addvars(out.statstable, sounddriven(:,1), sounddriven(:,2), pst(1).analrate, pst(2).analrate, ...
                pst(1).baselinerate, pst(2).baselinerate, halfmaxdur(:,1), halfmaxdur(:,2), ...
                ...
                'NewVariableNames', {'PulseDriven_1', 'PulseDriven_2', 'WindowRate1_Hz', 'WindowRate2_Hz', ...
                'BaselineRate1_Hz', 'BaselineRate2_Hz', 'Duration1_ms', 'Duration2_ms', ...
